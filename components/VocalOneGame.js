...
const VocalOneGame = ({
  ...
  const generarObjetosAleatorio = async () => {
    ...
    let arregloImagenes = [
      {
        nombre: "A",
        sources: [
          { img: img_avion, name: "Avión" },
          { img: img_arania, name: "Araña" },
          { img: img_arbol, name: "Árbol" },
        ],
      },
      {
        nombre: "E",
        sources: [
          { img: img_elefante, name: "Elefante" },
          { img: img_estrella, name: "Estrella" },
          { img: img_escoba, name: "Escoba" },
        ],
      },
      {
        nombre: "I",
        sources: [
          { img: img_iguana, name: "Iguana" },
          { img: img_isla, name: "Isla" },
          { img: img_iman, name: "Imán" },
        ],
      },
      {
        nombre: "O",
        sources: [
          { img: img_ojo, name: "Ojo" },
          { img: img_oso, name: "Oso" },
          { img: img_oreja, name: "Oreja" },
        ],
      },
      {
        nombre: "U",
        sources: [
          { img: img_uva, name: "Uva" },
          { img: img_unia, name: "Uña" },
          { img: img_uno, name: "Uno" },
        ],
      },
    ];
    ...
    const arregloLimpio = await arregloImagenes.filter(
    ...
    const arrFinal = [
      { source: source_secundario, key: objeto_secundario.nombre },
      { source: source_principal, key: objeto_principal.nombre },
    ];
  ...
  const validarCard = () => {
    ...
    if (vocalSeleccionada.key !== imgCard.key) {
      ...
      setDataAlert({
        ...
        detalle:
          "Esa vocal no es la correcta para esta imagen, inténtalo con la otra opción.",
        active: true,
        tipe: "validation",
      });
                ...
                >
                  <Card
                    style={{
                      ...styles.card,
                      borderColor:
                        vocalSeleccionada.key === obj.key ? "red" : "#000",
